...
const loadTests = async () => {
  try {
    ...
    console.error(e)
    error.value = e.message || 'Ошибка сервера'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="page-wrapper">
    ...
    <div v-if="loading" class="status-msg">Загрузка тестов...</div>
    <div v-if="error" class="error-msg">{{ error }}</div>

    <div v-if="!loading && tests.length === 0" class="status-msg">
    ...
    <div class="test-list">
      <div v-for="test in tests" :key="test.id" class="test-item">
        ...
        <button class="main-btn" @click="openTest(test.id)">Начать тест</button>
      </div>
    </div>
  </div>
</template>
